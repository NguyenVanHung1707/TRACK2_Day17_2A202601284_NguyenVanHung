...
{% macro priority_reject_reason(col) %}
    case
        ...
        when try_cast({{ col }} as integer) is not null and cast({{ col }} as integer) not between 1 and 4 then 'priority số ngoài khoảng 1..4'
        else 'priority nhãn chuỗi không hợp lệ'
    end
{% endmacro %}
